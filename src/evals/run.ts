/**
 * LangSmith Evaluation Runner for AlphaSentry
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps AlphaSentry agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `alphasentry-finance-eval-sample-${sampleSize}-${Date.now()}`
      : 'alphasentry-finance-eval';
    ...
    const experimentName = `alphasentry-eval-${Date.now().toString(36)}`;
